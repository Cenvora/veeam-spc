...
class BackupServerJob:
    ...
    Attributes:
        ...
        type_ (BackupServerJobType | Unset): Type of a job.
            > For backup copy jobs, the `BackupCopy` and `SimpleBackupCopy` values indicate the job engine. `BackupCopy`
            denotes the legacy engine. `SimpleBackupCopy` denotes the engine introduced in Veeam Backup & Replication 12 or
            later. These values do not reflect the copy mode, which can be periodic or immediate.
