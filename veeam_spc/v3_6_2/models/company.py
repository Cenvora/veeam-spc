...
class Company:
    ...
    Attributes:
        ...
        instance_uid (UUID | Unset): UID assigned to a company.
        name (str | Unset): Name of a company.
            > Can be changed using the `PatchOrganization` operation.
        status (CompanyStatus | Unset): Status of a service provider. You can set the `Active` or `Disabled` value using
            the PATCH method.
            > The following values are deprecated and remain for backward compatibility: `Expired`, `Creating`, `Deleting`,
            `Deleted`, `SiteResourceCreationFailed`, `SiteResourceCreating`, `SiteResourceUpdating`,
