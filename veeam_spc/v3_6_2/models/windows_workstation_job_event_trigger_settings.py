...
class WindowsWorkstationJobEventTriggerSettings:
    """
    Attributes:
        backup_on_lock (bool | Unset): Indicates whether a scheduled backup job must start when the user locks the
            computer. Default: False.
        backup_on_log_off (bool | Unset): Indicates whether a scheduled backup job must start when the user working with
            the computer performs a logout operation. Default: False.
        backup_on_target_connection (bool | Unset): Indicates whether a scheduled backup job must start when the backup
            storage becomes available. Default: False.
        eject_target_on_backup_complete (bool | Unset): Indicates whether Veeam Agent for Microsoft Windows must unmount
            the storage device after the backup job completes successfully.
            > Cannot be enabled if the `backupOnTargetConnection` property has the `false` value.
             Default: False.
        backup_not_often (int | Unset): Minutely, hourly or daily interval between the backup job sessions. Default: 2.
        not_often_time_unit (WindowsWorkstationJobEventTriggerSettingsNotOftenTimeUnit | Unset): Measurement units of
            interval between the backup job sessions. Default:
            WindowsWorkstationJobEventTriggerSettingsNotOftenTimeUnit.HOURS.
    """

    backup_on_lock: bool | Unset = False
    backup_on_log_off: bool | Unset = False
    backup_on_target_connection: bool | Unset = False
    eject_target_on_backup_complete: bool | Unset = False
    backup_not_often: int | Unset = 2
    not_often_time_unit: WindowsWorkstationJobEventTriggerSettingsNotOftenTimeUnit | Unset = (
        WindowsWorkstationJobEventTriggerSettingsNotOftenTimeUnit.HOURS
    )
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        not_often_time_unit: str | Unset = UNSET
