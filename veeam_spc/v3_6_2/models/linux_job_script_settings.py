...
class LinuxJobScriptSettings:
    """
    Attributes:
        enabled (bool | Unset): Indicates whether script processing is enabled. Default: False.
        pre_job_script (LinuxJobScript | Unset):
        post_job_script (LinuxJobScript | Unset):
        pre_freeze_script (LinuxJobScript | Unset):
        post_thaw_script (LinuxJobScript | Unset):
    """

    enabled: bool | Unset = False
    pre_job_script: LinuxJobScript | Unset = UNSET
    post_job_script: LinuxJobScript | Unset = UNSET
    pre_freeze_script: LinuxJobScript | Unset = UNSET
    post_thaw_script: LinuxJobScript | Unset = UNSET
    additional_properties: dict[str, Any] = _attrs_field(init=False, factory=dict)

    def to_dict(self) -> dict[str, Any]:
        enabled = self.enabled

        pre_job_script: dict[str, Any] | Unset = UNSET
        if not isinstance(self.pre_job_script, Unset):
            pre_job_script = self.pre_job_script.to_dict()

        post_job_script: dict[str, Any] | Unset = UNSET
        if not isinstance(self.post_job_script, Unset):
            post_job_script = self.post_job_script.to_dict()

        pre_freeze_script: dict[str, Any] | Unset = UNSET
        if not isinstance(self.pre_freeze_script, Unset):
            pre_freeze_script = self.pre_freeze_script.to_dict()

        post_thaw_script: dict[str, Any] | Unset = UNSET
